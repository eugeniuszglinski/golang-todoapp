...
func (r *TasksRepository) PatchTask(ctx context.Context, id int, task *domain.Task) (*domain.Task, error) {
	...
	row := r.pool.QueryRow(
		ctx, query, task.Title, task.Description, task.Completed, task.CompletedAt, task.ID, task.Version,
	)
	...
	if err != nil {
		if errors.Is(err, core_postgres_pool.ErrNoRows) {
			return nil, fmt.Errorf("task with id='%d' concurrently accessed: %w", id, core_errors.ErrConflict)
		}
